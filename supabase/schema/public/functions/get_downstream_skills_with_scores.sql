...
-- 
---------------------------------------------------------------------------
-- Function: public.get_downstream_skills_with_scores

---------------------------------------------------------------------------
-- BEGIN: PG_DUMP RESULT (FUNCTION: public.get_downstream_skills_with_scores)
------------------------------
CREATE OR REPLACE FUNCTION public.get_downstream_skills_with_scores(user_id text, input_skill_id text, start_date timestamp with time zone DEFAULT NULL::timestamp with time zone, end_date timestamp with time zone DEFAULT NULL::timestamp with time zone)
 RETURNS TABLE(skill_id text, skill_name text, path_from text[], path_from_links text[], min_normalized_score_downstream double precision, max_normalized_score_downstream double precision, average_normalized_score_downstream double precision, stddev_normalized_score_downstream double precision, activity_result_count_downstream bigint, all_scores double precision[], num_downstream_skills bigint, level_on_parent text, level_path text[])
 LANGUAGE plpgsql
AS $function$
BEGIN
    RETURN QUERY
...
------------------------------
-- END: PG_DUMP RESULT (FUNCTION: public.get_downstream_skills_with_scores)
---------------------------------------------------------------------------
