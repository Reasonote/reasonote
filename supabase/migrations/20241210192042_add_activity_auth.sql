----------------------------------------------------------------------------
----------------------------------------------------------------------------
-- activity Authorization Setup
----------------------------------------------------------------------------
----------------------------------------------------------------------------

--------------------------------------------------------------------------------
-- Insert the activity entity type (if needed)
--------------------------------------------------------------------------------
INSERT INTO public.entity_type (entity_type, abbreviation) VALUES 
  ('activity', 'actvty') 
ON CONFLICT (entity_type) DO NOTHING;

-- Attach the trigger to the activity table
DROP TRIGGER IF EXISTS grant_activity_creator_auth ON public.activity;
CREATE TRIGGER grant_activity_creator_auth
  AFTER INSERT ON public.activity
  FOR EACH ROW
  EXECUTE PROCEDURE public.tgr_grant_entity_creator_authorization('id', 'activity');

--------------------------------------------------------------------------------
-- Recreate vw_activity_memauth view
--------------------------------------------------------------------------------
...
JOIN public.activity l ON l.id = ma.resource_entity_id
...
COMMENT ON VIEW public.vw_activity_memauth IS 'Shows activity authorizations, what principals have which access_levels & permissions on which activity.';
GRANT SELECT ON public.vw_activity_memauth TO anon, authenticated, service_role;

--------------------------------------------------------------------------------
-- RLS policies on activity, using vw_activity_memauth for permission checks
--------------------------------------------------------------------------------
...
('activity.SELECT', 'Can select/view a activity'),
('activity.INSERT', 'Can insert/create a activity'),
('activity.UPDATE', 'Can update/edit a activity'),
('activity.DELETE', 'Can delete a activity'),
('activity.SHARE', 'Can grant or modify non-owner access levels on a activity'),
('activity.COMMENT', 'Can comment on a activity')
...
--  The `bar` table can then ask the activity permissions for `bar.SELECT` if it needs to.
--------------------------------------------------------------------------------

-- Owners can do anything on the activity.
INSERT INTO public.access_level_permission (entity_type, access_level, permission_code)
SELECT 'activity', 'owner', pc.permission_code
FROM (VALUES 
('activity.SELECT'), 
('activity.INSERT'), 
('activity.UPDATE'), 
...
-- Editors can select, update, and comment on a activity.
INSERT INTO public.access_level_permission (entity_type, access_level, permission_code)
SELECT 'activity', 'editor', pc.permission_code
FROM (VALUES 
('activity.SELECT'),
('activity.UPDATE'),
('activity.COMMENT')
...
-- Viewers can select a activity.
INSERT INTO public.access_level_permission (entity_type, access_level, permission_code)
VALUES ('activity', 'viewer', 'activity.SELECT')
ON CONFLICT (entity_type, access_level, permission_code) DO NOTHING;

-- Commenters can select a activity.
INSERT INTO public.access_level_permission (entity_type, access_level, permission_code)
SELECT 'activity', 'commenter', pc.permission_code
FROM (VALUES 
('activity.SELECT')
) AS pc(permission_code)
ON CONFLICT (entity_type, access_level, permission_code) DO NOTHING;
...
--------------------------------------------------------------------------------
-- Create activity memauth entries.
--------------------------------------------------------------------------------
INSERT INTO memauth (
    resource_entity_id,
    access_level,
