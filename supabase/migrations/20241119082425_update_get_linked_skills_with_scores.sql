DROP FUNCTION IF EXISTS public.get_linked_skills_with_scores(text, text, timestamp with time zone, timestamp with time zone);

CREATE FUNCTION public.get_linked_skills_with_scores(
    user_id text, 
    input_skill_id text, 
 ...
 RETURNS TABLE(
    ...
    stddev_normalized_score_upstream double precision, 
    activity_result_count_upstream bigint, 
    all_scores double precision[], 
    num_upstream_skills bigint, 
