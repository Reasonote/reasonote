  ...
  async createDocuments(
    ...
    for (let i = 0; i < texts.length; i += 1) {
      ...
      for (const chunk of await this.splitText(text)) {
        ...
        // Find the starting index of the chunk in the original text
        const indexChunk = text.indexOf(chunk, indexPrevChunk + 1);
  
        // Update line counters based on the number of newlines before the chunk
        if (prevChunk === null) {
        ...
        // Compute the character-level offsets
        const lineStartIndex = text.lastIndexOf("\n", indexChunk - 1) + 1;
        const firstLineOffset = indexChunk - lineStartIndex;
        loc.firstLineOffset = firstLineOffset;
        loc.rawOffset = indexChunk;
  
        const metadataWithOffsets = {
          ..._metadatas[i],
          loc,
        };
