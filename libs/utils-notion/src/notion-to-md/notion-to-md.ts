...
export class NotionToMarkdown {
  ...
  getFlatMarkdownBlocks(mdBlocks: MdBlock[] = [], nestingLevel: number = 0): { mdBlock: MdBlock, nestingLevel: number }[] {
    const ret: { mdBlock: MdBlock, nestingLevel: number }[] = [];

    mdBlocks.forEach((mdBlock) => {
      // process parent blocks
      if (mdBlock.parent) {
        ret.push({ mdBlock, nestingLevel });
      }

      // process child blocks
      if (mdBlock.children && mdBlock.children.length > 0) {
        ret.push(...this.getFlatMarkdownBlocks(mdBlock.children, nestingLevel + 1));
      }
    });
  ...
  toMarkdownString(mdBlocks: MdBlock[] = [], nestingLevel: number = 0): string {
    ...
    const flatMdBlocks = this.getFlatMarkdownBlocks(mdBlocks, nestingLevel);

    flatMdBlocks.forEach(({ mdBlock, nestingLevel }) => {
      if (mdBlock.parent) {
        if (
          mdBlock.type !== "to_do" &&
          mdBlock.type !== "bulleted_list_item" &&
          mdBlock.type !== "numbered_list_item"
        ) {
          // add extra line breaks non list blocks
          mdString += `\n${md.addTabSpace(mdBlock.parent, nestingLevel)}\n\n`;
        } else {
