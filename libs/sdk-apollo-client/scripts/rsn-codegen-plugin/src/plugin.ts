            ...
            if (d.kind === "ObjectTypeDefinition") {
                const filteredFields = d?.fields
                    ?.filter((f) => {
                        ...
                        // Flat fragments should be....
                        // ... scalar fields
                        if (f.type.kind === "NamedType") {
                            return totalScalarTypes.includes(f.type.name.value);
                        }
                        // ... Non null scalars are fine.
                        else if (f.type.kind === "NonNullType" && f.type.type.kind === "NamedType") {
                            return totalScalarTypes.includes(f.type.type.name.value);
                        }
                        // ....Scalar lists are fine.
                        else if (f.type.kind === "ListType" && f.type.type.kind === "NamedType") {
                            return totalScalarTypes.includes(f.type.type.name.value);
                        }
                        // .... Non null scalar lists are fine.
                        else if (f.type.kind === "NonNullType" && f.type.type.kind === "ListType" && f.type.type.type.kind === "NamedType") {
                            return totalScalarTypes.includes(f.type.type.type.name.value);
...
export const plugin: PluginFunction<ExpectedTotalConfig> = (schema: GraphQLSchema, documents, config, info) => {
    ...
    if (typeName === "fragment") {
        return handleFragment(schema, documents, config, info);
    } else if (typeName === "query") {
        return handleQuery(schema, documents, config, info);
    } else if (typeName === "mutation") {
        return handleMutation(schema, documents, config, info);
    }
    else if (typeName === "_rawPureSchema") {
        return JSON.stringify(pureSchema);
    } else if (typeName === "_rawPureSchemaGql") {
